...
@interface ZAHomeMenuCell()<UIScrollViewDelegate>
{
    UIView *_backView1;
    UIView *_backView2;
...
+(instancetype)cellWithTableView:(UITableView *)tableView menuArray:(NSMutableArray *)menuArray {
    ...
    ZAHomeMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if(cell == nil) {
        cell = [[ZAHomeMenuCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID menuArray:menuArray];
    }
    return cell;
}
    ...
    if(self) {
        ...
        for(int i = 0; i < 16; i++) {
            if(i < 4) {
                ...
                ZAMenuBtnView *btnView = [[ZAMenuBtnView alloc]initWithFrame:frame title:title imagestr:imagestr];
                btnView.tag = TAG + i;
